...
int KCPObject::kcp_output(const char* buf, int len, ikcpcb* kcp, void* user) {
	...
	if (thiz->context.IsEmpty()) {
		...
		Local<Value> argv[argc] = {Nan::CopyBuffer(buf, len).ToLocalChecked(), Nan::New<Number>(len), Nan::New<Object>(thiz->context)};
		Callback callback(Nan::New<Function>(thiz->output));
		callback.Call(argc, argv);
	}
	return len;
...
KCPObject::~KCPObject() {
	if (kcp) ikcp_release(kcp), kcp = NULL;
	if (recvBuff) free(recvBuff), recvBuff = NULL;
	if (!output.IsEmpty()) output.Reset();
	if (!context.IsEmpty()) context.Reset();
}
...
NAN_METHOD(KCPObject::New) {
	if (!info[0]->IsNumber()) {
		Nan::ThrowTypeError("KCP constructor arg 1 must be number");
		return;
	}
	if (info.IsConstructCall()) {
		uint32_t conv = To<uint32_t>(info[0]).FromJust();
		KCPObject* kcpobj = new KCPObject(conv);
		if (info[1]->IsObject()) kcpobj->context.Reset(Local<Object>::Cast(info[1]));
...
NAN_METHOD(KCPObject::GetContext) {
	...
	if (!thiz->context.IsEmpty()) info.GetReturnValue().Set(Nan::New<Object>(thiz->context));
}

NAN_METHOD(KCPObject::Release) {
	KCPObject* thiz = ObjectWrap::Unwrap<KCPObject>(info.Holder());
	if (thiz->kcp) ikcp_release(thiz->kcp), thiz->kcp = NULL;
	if (thiz->recvBuff) free(thiz->recvBuff), thiz->recvBuff = NULL;
}

NAN_METHOD(KCPObject::Recv) {
	KCPObject* thiz = ObjectWrap::Unwrap<KCPObject>(info.Holder());
	int bufsize = 0, buflen = 0, len = 0;
	...
	if (len > 0) info.GetReturnValue().Set(Nan::CopyBuffer((const char*)thiz->recvBuff, len).ToLocalChecked());
}
...
NAN_METHOD(KCPObject::Setmtu) {
	int mtu = 1400;
	if (info[0]->IsNumber()) mtu = To<int>(info[0]).FromJust();
	KCPObject* thiz = ObjectWrap::Unwrap<KCPObject>(info.Holder());
...
NAN_METHOD(KCPObject::Wndsize) {
	int sndwnd = 32;
	int rcvwnd = 32;
	if (info[0]->IsNumber()) sndwnd = To<int>(info[0]).FromJust();
	if (info[1]->IsNumber()) rcvwnd = To<int>(info[1]).FromJust();
	KCPObject* thiz = ObjectWrap::Unwrap<KCPObject>(info.Holder());
...
NAN_METHOD(KCPObject::Nodelay) {
	...
	int resend = 0;
	int nc = 0;
	if (info[0]->IsNumber()) nodelay = To<int>(info[0]).FromJust();
	if (info[1]->IsNumber()) interval = To<int>(info[1]).FromJust();
	if (info[2]->IsNumber()) resend = To<int>(info[2]).FromJust();
	if (info[3]->IsNumber()) nc = To<int>(info[3]).FromJust();
	KCPObject* thiz = ObjectWrap::Unwrap<KCPObject>(info.Holder());
